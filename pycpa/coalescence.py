...
from pycpa.utilities import histo
...
class Coalescence:
    ...
    def pseudo_neutron(proton, triton, helium3, bins=30, range=(0,600)):
        ...
        pandas.DataFrame
        """
        proton_spectra = dfhist.rebin1D(proton, bins=bins, range=range)
        triton_spectra = dfhist.rebin1D(triton, bins=bins, range=range)
        helium3_spectra = dfhist.rebin1D(helium3, bins=bins, range=range)
        return dfhist.div(helium3_spectra, dfhist.mul(proton_spectra, triton_spectra))
    ...
    def chemical_temperature(
        ...
        deuteron_spectrum = dfhist.rebin1D(deuteron, bins=bins, range=range)
        alpha_spectrum = dfhist.rebin1D(alpha, bins=bins, range=range)
        triton_spectrum = dfhist.rebin1D(triton, bins=bins, range=range)
        helium3_spectrum = dfhist.rebin1D(helium3, bins=bins, range=range)
    ...
    def coalescence_spectra(spectra, type:Literal['p','n']='p', range=(0, 600), bins=30):
        ...
        for zn, df in spectra.items():
            rebinned_spectra[zn] = dfhist.rebin1D(df, range=range, bins=bins)
